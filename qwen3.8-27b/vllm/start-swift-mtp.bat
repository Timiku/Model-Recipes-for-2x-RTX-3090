...
title model-recipes serve - qwen3.8-27b / swift-mtp.yml (swift-mtp tier)
...
set "TIER=swift-mtp"
set "YML=swift-mtp.yml"
set "PORT=8113"
set "CNAME=qwen-27b-swift-serve"
set "WAIT=5"
...
echo  stance: SWIFT - the Swift checkpoint (W4A16, compressed-tensors,
echo          its own 25,879-token draft head + int8 embeddings) on the
echo          mtp body: built-in MTP drafter n=4, fp8 KV, the same
echo          serve shape as the mtp tier. Measured 09-18/19: pool
echo          654,635 tokens (2.50x at 262k), NO depth cliff (TPOT
echo          22-27 ms from 4k to 210k), acceptance 40-46 percent,
echo          AL 2.6-2.9. The drafter is tuned to Swift's own output
echo          distribution, so its upstream win shows on real
echo          coding-agent text, not on fixed random prompts.
echo          Sibling: start-mtp.bat - the base checkpoint, SAME
echo          cards, port 8113. Only one of the two runs at a time;
echo          stop this tier with stop.bat (or close this window)
echo          before booting the other.
echo.
