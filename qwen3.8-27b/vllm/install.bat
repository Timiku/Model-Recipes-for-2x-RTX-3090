...
title model-recipes install - qwen3.8-27b
...
echo   written DEVICE_PAIR=!PAIR! to all seven tier machine .envs
goto :dev-done
:dev-done
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%MCFG%" "%~dp0superfast.env" BIND_HOST "!SB_VAL!" >nul
...
echo  if a model ships a fetch script, downloads any missing folder:
if defined WD_STAND (
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%MCFG%" "%~dp0superfast.env" WEIGHTS_DIR "!WD_VAL!" >nul
