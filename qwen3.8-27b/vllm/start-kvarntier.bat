...
title model-recipes serve - qwen3.8-27b / kvarntier.yml (KVarN tier)
...
set "TIER=kvarntier"
set "YML=kvarntier.yml"
set "PORT=8116"
set "CNAME=qwen-27b-kvarn-serve"
set "WAIT=5"
...
echo  stance: KVarN - int4 KV cache (kvarn_k4v2_g128), drafter OFF
echo          (measured: pool 1,450,530 tokens, 5.53x; needle HIT at 100k
echo          and 240k; 34.4 / 44.1 / 53.0 / 52.4 tok/s across the ladder).
...
echo          KVARN decode path (measured 09-15; see MODEL.md). Keep
echo          MAX_NUM_SEQS as shipped, and serve long-ctx concurrency on
echo          the fp8 tiers.
echo          Siblings: start-kvarnmtp.bat - the SAME port and SAME cards,
echo          drafter ON at n=4, 1,113,340 tokens; run one shape at a time.
echo          Also start-kvarndflash2.bat (same KV, DFlash2 drafter),
echo          start-nomtp.bat (fp8).
echo.
