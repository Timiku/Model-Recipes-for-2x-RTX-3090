...
title model-recipes install - gemma4-31b
...
rem (The KVarN tiers are dev-tree research only - the head-512 SDPA slow path
rem makes deep-ctx decode unusable, 0.3 tok/s at 100k; they do not ship.)
...
echo.
rem ------------------------------------------------------------------
rem  stage-4 contract - the weights: a folder missing on this box is
rem  FETCHED by the installer (vllm\package\weights-source.sh, resume-safe):
rem    hf download cyankiwi/gemma-4-31B-it-AWQ-4bit - the target
rem    hf download google/gemma-4-31B-it-assistant - the MTP drafter
rem  each into the model's WEIGHTS_DIR in the Hugging Face repo layout.
rem  The wizard's first run leaves WEIGHTS_DIR blank (the model's own
rem  weights folder in this tree); a re-run reads the standing line. Both
rem  repos are public and ungated. Only if the fetch cannot run does
rem  stage 4 pause with these same commands left for you.
rem ------------------------------------------------------------------
...
if defined HAVE_DISTRO (
    echo   [--] 8. disk in the distro - the image pulls + the weights want
    echo        ~40 GB headroom:
...
set "PAIR_STAND="
for /f "delims=" %%A in ('powershell -NoProfile -ExecutionPolicy Bypass -File "%MGET%" "%~dp0gemma-dual.env" DEVICE_PAIR 2^>nul') do set "PAIR_STAND=%%A"
echo.
...
goto :dev-done
:dev-write
for %%T in (gemma-dual gemma-dual-nomtp) do powershell -NoProfile -ExecutionPolicy Bypass -File "%MCFG%" "%~dp0%%T.env" DEVICE_PAIR "!PAIR!" >nul
echo   written DEVICE_PAIR=!PAIR! to both tier machine .envs
goto :dev-done
:dev-done
rem ---- the serve bind: the raw address the tier binds to ----
set "BIND_STAND="
for /f "delims=" %%A in ('powershell -NoProfile -ExecutionPolicy Bypass -File "%MGET%" "%~dp0gemma-dual.env" BIND_HOST 2^>nul') do set "BIND_STAND=%%A"
set "SB_VAL="
echo.
...
echo    127.0.0.1  the loopback only (nothing off-box reaches it; on a
...
goto :bind-done
:bind-write
powershell -NoProfile -ExecutionPolicy Bypass -File "%MCFG%" "%~dp0gemma-dual.env" BIND_HOST "!SB_VAL!" >nul
powershell -NoProfile -ExecutionPolicy Bypass -File "%MCFG%" "%~dp0gemma-dual-nomtp.env" BIND_HOST "!SB_VAL!" >nul
...
set "WD_STAND="
for /f "delims=" %%A in ('powershell -NoProfile -ExecutionPolicy Bypass -File "%MGET%" "%~dp0gemma-dual.env" WEIGHTS_DIR 2^>nul') do set "WD_STAND=%%A"
set "WD_VAL="
echo.
echo  weights download path - stage 4 verifies the weight folders here and,
echo  if the model ships a fetch script, downloads any missing folder:
if defined WD_STAND (
...
goto :wd-done
:wd-write
powershell -NoProfile -ExecutionPolicy Bypass -File "%MCFG%" "%~dp0gemma-dual.env" WEIGHTS_DIR "!WD_VAL!" >nul
