...
port(0.27.1): ported from KVarN (vLLM v0.23.0 fork) onto vLLM v0.27.1. Every
port change is marked with a ``# port(0.27.1): ...`` comment; see the port
plan (kvarn_plan.md §2.1 items 1-12) for the rationale of each hunk.
"""
...
from vllm.v1.attention.ops.kvarn_store import (
    # port(0.27.1): the single-tile kvarn_store_tile_k/v were imported but
    # unused (plan §1 marks them as deletable in kvarn_store.py).
...
def _active_kvarn_cache_dtype() -> str | None:
    """port(0.27.1): the ``kvarn_*`` cache dtype of the CURRENT vLLM config,
    falling back to ``KVarNAttentionImpl._active_cache_dtype`` (the preset of
    the impl constructed for this deployment) when no config is set or its
    cache_dtype is not a KVarN preset; None only if neither is available.
    0.27.1's ``get_current_vllm_config()`` raises outside a
    ``set_current_vllm_config`` context, so use the ``_or_none`` variant."""
    ...
    if cfg is None:
        # port(0.27.1): no current config (e.g. called from a worker thread /
        # unit test outside set_current_vllm_config) — fall back to the preset
        # recorded by the impl constructed for this deployment (module-level
        # class, resolved at call time).
        return KVarNAttentionImpl._active_cache_dtype
    try:
    ...
    if isinstance(cache_dtype, str) and cache_dtype in KVARN_PRESETS:
        return cache_dtype
    # port(0.27.1): config present but cache_dtype is not a KVarN preset (e.g.
    # "auto") — same impl-recorded fallback.
    return KVarNAttentionImpl._active_cache_dtype


def _active_kvarn_group() -> int | None:
    """port(0.27.1): tile size (== kernel block size) of the active preset."""
    from vllm.model_executor.layers.quantization.kvarn.config import (
        KVARN_PRESETS,
    )

    cache_dtype = _active_kvarn_cache_dtype()
    return KVARN_PRESETS[cache_dtype]["group"] if cache_dtype else None


def _causal_bool(c: Any) -> bool:
    """port(0.27.1): ``CommonAttentionMetadata.causal`` is ``bool | Tensor``
    in 0.27.1 (plan §2.1.8). KVarN has no per-row causality, so a tensor is
    reduced with all() (one D2H sync; the V1 runner passes a plain bool)."""
    if isinstance(c, torch.Tensor):
        return bool(c.all().item())
    return bool(c)


def _scratch_groups(
    seq_lens: list[int], rows: int,
) -> list[tuple[int, int]] | None:
    """house adaptation #4 (2026-09-15): partition request rows into groups
    whose summed live context fits the shared materialize scratch (``rows``).

    The scratch holds ONE packed write of ``rows`` tokens and the batch is
    packed into it as a single varlen run, so a batch summing past ``rows`` has
    nowhere to go in the fast path. Splitting it into scratch-fitting groups
    lets the same packed-kv kernel and the same ``flash_attn_varlen`` call run
    once per group: FlashAttention tiles each sequence independently, so a
    group boundary cannot change any sequence's result. Greedy in row order —
    the grouping is a pure function of the batch, which is what keeps it safe
    outside a captured region.

    Returns ``None`` when a single request is wider than the scratch: it cannot
    be packed at all, and the caller keeps the fp32 fallback for that batch.
    """
    if rows <= 0:
        return None
    ...
    for r, s in enumerate(seq_lens):
        ...
        if acc + s > rows:
            ...
            acc = 0
        acc += s
    groups.append((start, len(seq_lens)))
    return groups
...
class KVarNAttentionBackend(AttentionBackend):
    ...
    def get_supported_kernel_block_sizes() -> list[int | MultipleOf]:
        # One vLLM block == one KVarN tile (cfg.group).
        # port(0.27.1): report ONLY the active preset's group (plan §2.1.2).
        # 0.27.1's select_common_block_size (worker/utils.py) picks the LARGEST
        # int size dividing the (hybrid-aligned) manager block, so advertising
        # every preset's group would hand a g64 preset a 128-token kernel block
        # and trip the block_size == group assert. It also makes
        # _align_hybrid_block_size's kernel alignment (min of the list) equal
        # to the preset's group. Fallback (no config set, e.g. unit tests):
        # the distinct `group` values across the registered presets (64, 128).
        group = _active_kvarn_group()
    ...
    def get_preferred_block_size(cls, default_block_size: int) -> int:
        ...
        # port(0.27.1): same lookup as get_supported_kernel_block_sizes, via
        # get_current_vllm_config_or_none (get_current_vllm_config raises).
        group = _active_kvarn_group()
        if group is not None:
            return group
    ...
    def get_kv_cache_shape(
        ...
        # port(0.27.1): belt-and-braces against the 'auto' trap (plan §2.1.1).
        # gpu_model_runner._reshape_kv_cache_tensors passes cache_dtype_str=
        # "auto" whenever spec.kv_quant_mode == KVQuantMode.NONE; the primary
        # fix is KVQuantMode.KVARN (kv_cache_interface.py), but if a caller
        # still hands us a non-KVarN string, resolve the active preset from the
        # impl that was constructed for this deployment, else from the current
        # vLLM config, instead of raising in from_cache_dtype.
        if cache_dtype_str not in KVARN_PRESETS:
            resolved = (
                KVarNAttentionImpl._active_cache_dtype
                or _active_kvarn_cache_dtype()
            )
            if resolved is None:
                raise ValueError(
                    f"KVarNAttentionBackend.get_kv_cache_shape: cannot resolve "
                    f"a KVarN preset from cache_dtype_str={cache_dtype_str!r} "
                    "(no KVarN impl constructed and no current vLLM config)."
                )
            cache_dtype_str = resolved
    ...
    def get_kv_cache_stride_order(
        ...
        # port(0.27.1): declare the physical layout (plan §2.1.3). The logical
        # shape (num_blocks, num_kv_heads, tile_bytes_aligned) IS the physical
        # layout — blocks first. With a prepended num_layers dim the physical
        # order is (num_blocks, num_layers, num_kv_heads, tile), i.e. still
        # blocks-first, so AttentionBackend.indexes_kv_by_block_stride() is
        # True and 0.27.1's generic page padding in unify_kv_cache_spec_page_
        # size (kv_cache_utils.py) is legal for KVarN layers (the kernels read
        # kv_cache.stride(0)/stride(1), so a padded block stride is fine).
        # Not exercised on Qwen3.8 (attention page == padded mamba page after
        # _align_hybrid_block_size); safety net that replaces KVarN's 0.23
        # kv_cache_utils.py hunk.
...
class KVarNMetadataBuilder(AttentionMetadataBuilder[KVarNMetadata]):
    ...
    # port(0.27.1): owner registry (plan §2.1.5) — layer_name -> the builder
    # whose build() drives that layer's impl (slot allocation, sink marking,
    # tile-boundary flush). Under MTP the target group's builder is created
    # with ALL layers of the KV-cache group INCLUDING the draft layer
    # (gpu_model_runner.initialize_attn_backend), and the drafter then creates
    # its own KVarN builder for the same draft layer (llm_base_proposer.
    # initialize_attn_backend). Without single ownership both builders re-tag
    # impl._group_key and flush the draft impl from different slot maps ->
    # draft KV silently wrong -> lower acceptance. A layer is claimed by a
    # builder if unowned or if the claimant has no MORE layers than the current
    # owner (so the drafter's 1-layer builder wins the draft layer, and a
    # re-created same-size builder — builders are re-created on every
    # initialize_kv_cache, incl. the profiling one — takes over its
    # predecessor's layers).
    _owner: ClassVar[dict[str, "KVarNMetadataBuilder"]] = {}
    ...
    def __init__(self, kv_cache_spec, layer_names, vllm_config, device):
        ...
        # port(0.27.1): cross-check (plan §2.1.12). 0.27.1 hands the builder
        # kv_cache_spec.copy_with_new_block_size(kernel_block_size) (worker/
        # utils.py create_metadata_builders), i.e. the KERNEL block size — the
        # hybrid-aligned MANAGER block (e.g. 1664) is split into kernel blocks
        # by the runner and block tables / slot_mapping arrive in kernel-block
        # units. One kernel block must be exactly one KVarN tile.
        _spec_bs = getattr(kv_cache_spec, "block_size", None)
        assert _spec_bs is None or _spec_bs == self._group, (
            f"KVarN: kernel block_size ({_spec_bs}) != tile group "
            f"({self._group}); check get_supported_kernel_block_sizes / "
            "--block-size."
        )

        # port(0.27.1): builder-side impl discovery (plan §2.1.4). Resolve this
        # group's KVarN impls straight from the static forward context instead
        # of matching an impl.layer_name attribute (which needed an
        # attention.py hunk in the 0.23 fork). Then claim them in the owner
        # registry (plan §2.1.5) and reset the per-group allocator state
        # (plan §2.1.6) — builders are (re)constructed on every
        # initialize_kv_cache (profiling KV cache -> real KV cache), so any
        # slot map / sink set / _kv_cache_ref left over from the previous
        # KV cache is stale here.
        # _impl_by_name = the impls this builder claimed at construction; the
        # authoritative set is re-derived from _owner in _owned_impls() at
        # build() time, because a later-constructed builder (the drafter's)
        # may take a layer over.
        self._impl_by_name: dict[str, "KVarNAttentionImpl"] = {}
        try:
            from vllm.config import get_layers_from_vllm_config
            from vllm.model_executor.layers.attention.attention import Attention

            _layers = get_layers_from_vllm_config(
                vllm_config, Attention, self._layer_names
            )
        except Exception:
            _layers = {}
        for name in self._layer_names:
            layer = _layers.get(name)
            impl = getattr(layer, "impl", None) if layer is not None else None
            if not isinstance(impl, KVarNAttentionImpl):
                continue
            owner = KVarNMetadataBuilder._owner.get(name)
            if (owner is None or owner is self
                    or len(self._layer_names) <= len(owner._layer_names)):
                KVarNMetadataBuilder._owner[name] = self
                self._impl_by_name[name] = impl
        KVarNAttentionImpl._reset_group_state(
            self._group_key, list(self._impl_by_name.values()))
    ...
    def _owned_impls(self) -> list["KVarNAttentionImpl"]:
        """port(0.27.1): the impls whose CURRENT owner (plan §2.1.5) is this
        builder — the only ones build() may allocate slots for / flush."""
        return [
    ...
    def build_for_drafting(
        ...
        # port(0.27.1): drafter passes (plan §2.1.5 follow-through). The MTP
        # drafter's first pass (draft_index == 0) reuses the target's metadata,
        # so committed = seq_len - query_len is the accepted length and the
        # normal flush logic is safe. Later passes (draft_index >= 1) run one
        # draft token per request with seq_lens = accepted + draft_index, so
        # `committed` OPTIMISTICALLY counts earlier draft tokens (rejectable
        # next step) as final: flushing on that could freeze a block holding a
        # to-be-rejected draft token into int4 for the draft layer, and the
        # rewrite after rejection would re-slot the block with an empty pool
        # slot (its history then reads as zeros). So for draft_index >= 1 only
        # ALLOCATE pool slots for the blocks the drafter writes; leave flush /
        # reclaim / sink bookkeeping to the next target or first-pass build.
        return self.build(
            0, common_attn_metadata, fast_build=True,
            _allow_flush=(draft_index == 0),
        )

    def build(self, common_prefix_len, common_attn_metadata, fast_build=False,
              _allow_flush: bool = True):
        ...
        # port(0.27.1): keep EXACT lengths (plan §2.1.9) — use the runner's
        # cached CPU copy when present (cam._seq_lens_cpu; None under async
        # spec decode), else an explicit device->host copy. Do NOT use
        # seq_lens_cpu_upper_bound (optimistic under async spec decode: the
        # commit/flush decisions below need exact seq_len - query_len), and
        # avoid the @deprecated seq_lens_cpu property.
        ...
        # port(0.27.1): CommonAttentionMetadata.causal is `bool | torch.Tensor`
        # in 0.27.1 (plan §2.1.8). Normalise ONCE to a Python bool for the vq
        # plan and the FA kwargs. The V1 runner always passes True (a Python
        # bool); a per-request tensor (all rows must agree — KVarN has no
        # per-row causality) costs one D2H sync here, never in forward().
        causal_flag = _causal_bool(getattr(cam, "causal", True))
        ...
        gk = self._group_key
        # port(0.27.1): THIS group's impls = the ones this builder currently
        # OWNS (resolved via get_layers_from_vllm_config in __init__, plan
        # §2.1.4, and filtered through the owner registry, plan §2.1.5) —
        # not a layer_name match over _all_impls. Tag them with the true group
        # key, so their _ensure_pool / store paths use this group's slot
        # allocator + mirror.
        group_impls = self._owned_impls()
        ...
        if group_impls:
            ...
            # ── KVarN diag (KVARN_BT_DEBUG): validate the block ids this step
            # will hand to the decode kernels. The decode/build-packed kernels
            # read the tile at block_id UNMASKED (only the pool-slot lookup is
            # bounded), so any id >= the int4 cache's block count is an
            # out-of-bounds read -> illegal memory access. Also compare the
            # per-request block count against the block table's row width: a
            # count wider than the row reads past the row (and past the tensor
            # for the last request).
            if os.environ.get("KVARN_BT_DEBUG") == "1" and max(
                    seq_lens_cpu[:B] or [0]) > 1000:
                _n = KVarNAttentionImpl.__dict__.get("_bt_dbg_n", 0)
                if _n < 1200:
                    KVarNAttentionImpl._bt_dbg_n = _n + 1
                    _ncache = int(getattr(KVarNAttentionImpl,
                                          "_kv_cache_num_blocks", 0) or 0)
                    _lookup = int(getattr(impl0, "_block_lookup_size", 0) or 0)
                    _nb = [(int(s) + GROUP - 1) // GROUP
                           for s in seq_lens_cpu[:B]]
                    _mx = 0
                    _mn = 1 << 30
                    _neg = 0
                    _over_cache = 0
                    _over_lookup = 0
                    for _b in range(min(B, bt_rows)):
                        _k = min(_nb[_b], bt_cols)
                        if _k <= 0:
                            continue
                        _seg = block_table_np[_b][:_k]
                        _mx = max(_mx, int(_seg.max()))
                        _mn = min(_mn, int(_seg.min()))
                        _neg += int((_seg < 0).sum())
                        if _ncache:
                            _over_cache += int((_seg >= _ncache).sum())
                        if _lookup:
                            _over_lookup += int((_seg >= _lookup).sum())
                    _fa_blocks = (int(self._max_model_len) + GROUP - 1) // GROUP
                    _scratch = (int(impl0._fa_K_buf.shape[0])
                                if getattr(impl0, "_fa_K_buf", None) is not None
                                else -1)
                    _tot = sum(int(s) for s in seq_lens_cpu[:B])
                    print(
                        f"[KVARN_BT] {_n} B={B} bt_rows={bt_rows} bt_cols={bt_cols} "
                        f"fa_blocks={_fa_blocks} n_blocks={_nb} "
                        f"id_min={_mn} id_max={_mx} neg={_neg} cache_blocks={_ncache} "
                        f"over_cache={_over_cache} over_lookup={_over_lookup} "
                        f"total_k={_tot} scratch={_scratch} "
                        f"over_scratch={'yes' if (_scratch >= 0 and _tot > _scratch) else 'no'} "
                        f"blocks_gt_cols={sum(1 for x in _nb if x > bt_cols)} "
                        f"sl={seq_lens_cpu[:B]} ql={query_lens_cpu[:B]}",
                        flush=True,
                    )
            ...
            # port(0.27.1): drafter passes >= 1 carry an OPTIMISTIC committed
            # length (see build_for_drafting) — no flush / reclaim there.
            for b in (range(B) if _allow_flush else ()):
                if b >= bt_rows or bt_cols == 0:
                    break
                sl = seq_lens_cpu[b]
                row = block_table_np[b]
            ...
            for bid in ([b for b in dict_map
                        ...
                        if _allow_flush else ()):   # port(0.27.1): see (2)
        ...
        if num_decodes > 0 and num_decode_tokens > num_decodes:
            ...
            non_causal = not causal_flag   # port(0.27.1): normalised bool
            i = 0
            ...
            for b in range(num_decodes):
                ...
                for j in range(ql):
                    ...
                    i += 1
            # port(0.27.1): padded uniform batches under CUDA-graph
            # capture/replay can leave num_decode_tokens > sum(query_lens);
            # zero the tail so the H2D copy below never ships garbage from
            # the torch.empty pinned buffers into the kernel's per-row plan.
            if i < num_decode_tokens:
                self._vq_seqlen_host[i:num_decode_tokens] = 0
                self._vq_req_host[i:num_decode_tokens] = 0
...
class KVarNAttentionImpl(AttentionImpl["KVarNMetadata"]):
    ...
    # port(0.27.1): AttentionImplBase defaults supports_dcp=True; KVarN's
    # decode kernels return no LSE and know nothing about decode-context-
    # parallel KV sharding (plan §4 'Selector'). PCP is False by default.
    supports_dcp: bool = False

    # port(0.27.1): the kvarn_* cache dtype this deployment's impls were built
    # with — read by get_kv_cache_shape when a caller passes a non-KVarN
    # cache_dtype_str (the 'auto' trap, plan §2.1.1).
    _active_cache_dtype: ClassVar[str | None] = None
    ...
    # #8b: persistent gather-output buffers for _decode_path_slow, keyed
    # (device, head_size, num_kv_heads) like the FA scratch.
    _shared_slow_K: ClassVar[dict[torch.device, torch.Tensor]] = {}
    _shared_slow_V: ClassVar[dict[torch.device, torch.Tensor]] = {}
    ...
    def _reset_group_state(
        cls, group_key: tuple, impls: list["KVarNAttentionImpl"]
    ) -> None:
        """port(0.27.1): drop all per-group allocator state (plan §2.1.6).

        Called from KVarNMetadataBuilder.__init__, i.e. whenever the builder
        for a KV-cache group is (re)constructed. In 0.27.1 that happens on
        every initialize_kv_cache — first for the minimal PROFILING KV cache
        (used for the CUDA-graph memory estimate) and again for the REAL one —
        and _cleanup_profiling_kv_cache never clears KVarN's cached
        impl._kv_cache_ref (it only knows _k_scale_cache/_v_scale_cache). So
        at builder construction the block->slot map, sink set, GPU mirrors,
        per-impl tail trackers and the cached kv_cache reference all describe a
        KV cache (block-id space) that no longer exists. Pool tensors, shared
        scratch and the compiled-kernel registry are kept (still valid; only
        allocated once). The next _ensure_pool call re-creates the free list /
        mirrors for `group_key`; the next forward() re-caches _kv_cache_ref
        (_batched_flush already skips impls whose ref is None).
        """
        cls._block_to_slot_dict.pop(group_key, None)
        cls._free_slots.pop(group_key, None)
        cls._allocator_pool_size.pop(group_key, None)
        cls._global_sink_blocks.pop(group_key, None)
        cls._max_known_block_id.pop(group_key, None)
        for mkey in [k for k in cls._block_to_slot_t_per_device
                     if k[1] == group_key]:
            cls._block_to_slot_t_per_device.pop(mkey, None)
            cls._is_sink_t_per_device.pop(mkey, None)
        for impl in impls:
            impl._group_key = group_key
            impl._tails.clear()
            impl._sink_blocks.clear()
            impl._kv_cache_ref = None
            # Per-instance mirror pointers are re-bound by the next
            # _ensure_pool; drop them so a stale mirror can never be launched.
            impl._block_to_slot_t = None
            impl._is_sink_t = None
            impl._block_lookup_size = 0
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # port(0.27.1): remember the active preset for get_kv_cache_shape's
        # 'auto' fallback (plan §2.1.1).
        type(self)._active_cache_dtype = kv_cache_dtype
        ...
        # port(0.27.1): SM count, filled by _ensure_pool (plan §2.1.11).
        ...
        # port(0.27.1): number of speculative tokens (0 = no spec decode),
        # cached here because impls are constructed under
        # set_current_vllm_config (model load) while _warm_decode_kernels may
        # run outside it (get_current_vllm_config raises there in 0.27.1).
        self._num_speculative_tokens: int = 0
        try:
            ...
            _spec = _cfg.speculative_config
            self._num_speculative_tokens = int(
                getattr(_spec, "num_speculative_tokens", 0) or 0
    ...
    def _ensure_pool(self, device: torch.device, num_blocks_hint: int = 0) -> None:
        ...
        # port(0.27.1): cache the SM count once per impl (plan §2.1.11). The
        # decode / verify drivers read `impl._sm_count` for the split-K
        # decision and re-query cudaGetDeviceProperties on every call when it
        # is unset.
        if not getattr(self, "_sm_count", 0):
        ...
        # port(0.27.1): make the cap configurable (KVARN_FA_SCRATCH_CAP, plan
        # §2.1.10) and apply it to the max_model_len term too — the original
        # max(..., max_model_len) forced 1.0 GiB at max_model_len 262144
        # regardless of the cap. Trade-off, resolved 2026-09-15: a batch whose
        # total context exceeds the cap used to take _decode_path_slow in
        # _cached_multiquery_path (chunked-prefill continuation / short-context
        # verify) and OOM there; house adaptation #4 now splits such a batch
        # into scratch-fitting request groups and stays on the fast path. The
        # slow path is left for a single request wider than the scratch and for
        # head_dim > 256; the fused decode / verify kernels never touch this
        # scratch.
        # port(0.27.1): sizing lives in KVarNConfig.fa_scratch_rows (single
        # tunable shared with the platform's memory accounting) — no inline
        # env read / max-min arithmetic here.
        from vllm.model_executor.layers.quantization.kvarn.config import (
            KVarNConfig,
        )
        fa_rows = KVarNConfig.fa_scratch_rows(self._max_num_seqs,
                                              self._max_model_len)
        cls = type(self)
    ...
    def _slow_kv_bufs(self, seq_len: int, device: torch.device):
        """#8b: persistent gather-output buffers for the slow path.

        The decode step at depth allocated seq_len x Hk x D fp16 K/V
        fresh every layer every token -- at 100k that is ~8.2 GiB of
        per-step alloc/free churn on the box whose WDDM GPU-PV allocator
        surfaced exactly this pattern once before as the #5b v2
        commit-pool ceiling (dxgkio_make_resident -ENOMEM, 09-16). The
        09-21 bench measured deep decode at 0.4 tok/s with the streaming
        and gather math itself at ~26 ms/layer -- the rest of the step
        was the churn. Class-shared per (device, D, Hk) like the FA
        scratch; grows in 4096-token steps to the largest context seen
        and never shrinks. The slow path is eager-only (never graph
        captured), so a persistent buffer is safe. fp16 fixed: the
        forward() entry casts activations to fp16 before any sub-path.
        """
        cls = type(self)
        bkey = (device, self.head_size, self.num_kv_heads)
        cur = cls._shared_slow_K.get(bkey)
        if cur is None or cur.shape[0] < seq_len:
            cap = ((seq_len + 4095) // 4096) * 4096
            cls._shared_slow_K[bkey] = torch.empty(
                cap, self.num_kv_heads, self.head_size,
                dtype=torch.float16, device=device)
            cls._shared_slow_V[bkey] = torch.empty_like(
                cls._shared_slow_K[bkey])
        return (cls._shared_slow_K[bkey][:seq_len],
                cls._shared_slow_V[bkey][:seq_len])
    ...
    def _warm_decode_kernels(self, device: torch.device) -> None:
        ...
        # port(0.27.1): no MAX_BLOCKS_PER_REQ in `common` — the fused decode /
        ...
        # port(0.27.1): that kernel is only used with KVARN_SHARED_VERIFY=1
        # (default OFF — latent corruption, plan §4 'MTP'), so gate the warmup
        # on it too instead of paying its compile + autotune sweep (and its
        # local-memory reservation) for nothing. Read the spec config via the
        # value cached at impl construction (get_current_vllm_config() raises
        # outside a set_current_vllm_config context in 0.27.1, e.g. at profile
        # time, which is when this warmup runs).
        _qlen = (1 + self._num_speculative_tokens
                 if self._num_speculative_tokens > 0 else 0)
        # port(0.27.1): mirror the driver's guard in
        # triton_kvarn_decode.kvarn_verify_attention — the shared-dequant
        # kernel's Q tile is [QLEN * Q_PER_KV_PAD, D] and tl.arange needs a
        # power-of-2 row count; if the driver would never launch it for this
        ...
        # 4. Packed-direct decode kernel (#8d) — same constexprs as
        # launch_prefill_direct so the warm variant serves.
        from vllm.v1.attention.ops.triton_kvarn_prefill_direct import (
            _kvarn_prefill_direct_kernel,
            DIRECT_BM, DIRECT_BN, DIRECT_WARPS, DIRECT_STAGES,
        )
        _grid_m = (B + DIRECT_BM - 1) // DIRECT_BM
        _kvarn_prefill_direct_kernel[(_grid_m, Hk)](
            q, bt[0:1], sl[0:1], b2s, cache, pool_k, pool_v, out, self.scale,
            B,
    ...
    def do_kv_cache_update(
        ...
        # KVarN diag (KVARN_BT_DEBUG): remember the true cache block count so the
        # metadata builder can validate every block id it hands to the decode
        # kernels. An out-of-cache id indexes the int4 cache out of bounds (the
        # slot lookup is masked, the tile read is not) -> illegal memory access.
        if kv_cache.numel():
            type(self)._kv_cache_num_blocks = int(kv_cache.shape[0])
    ...
    def forward(
        ...
        # port(0.27.1): do this BEFORE the attn_metadata-is-None early return
        # (plan §2.1.7). profile_run() calls forward with attn_metadata=None
        # and no slot mapping (unified_kv_cache_update is skipped), so this is
        # the only KVarN code that runs inside the measured profiling window;
        # materialising the fp16 tail pools, the shared decode/FA scratch and
        # the Triton JIT/autotune here charges them to non_kv_cache_memory
        # (memory_profiling's free-memory delta) instead of the CUDA-graph
        # estimate — no gpu_worker.py hunk needed. Use query.device: during
        # profiling kv_cache is still the CPU placeholder torch.tensor([]) (its
        # shape[0] is 0, so the hint is harmless).
        self._ensure_pool(device, num_blocks_hint=kv_cache.shape[0])
    ...
    def _sdpa_grouped(
        ...
        **kwargs,
        ...
        """SDPA without `enable_gqa`. torch's fused SDPA backends reject
        mismatched q/kv head counts ("both fused kernels require query, key
        and value to have the same num_heads"), and the math fallback they
        leave materializes the full q x k score matrix — exactly 1 GiB fp32
        for gemma-4's first 4096-token chunk (Hq=16/rank, head 512), which
        was the needle OOM of 2026-09-16: `_prefill_first_chunk` -> SDPA ->
        "Tried to allocate 1024.00 MiB". Grouping the q heads per kv head and
        broadcasting K/V (stride-0 expand) keeps the mem-efficient kernel
        engaged, which tiles instead of materializing. House adaptation #5;
        probed inside the v0.29.0 container (sm_86): q=4096 x ctx=189000 at
        head 512 runs +128 MiB maskless/is_causal and +1.6 GiB with a bool
        mask, against +2.9 GiB for the math backend at q=k=4096. Same head
        mapping as enable_gqa (q head h -> kv head h // G)."""
        ...
        for g in range(self.num_kv_heads):
            outs.append(F.scaled_dot_product_attention(
                q[:, g * G:(g + 1) * G],
                k[:, g:g + 1].expand(-1, G, -1, -1),
                v[:, g:g + 1].expand(-1, G, -1, -1),
                **kwargs,
            ))
    ...
    def _sdpa_streaming(
        self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor,
        scale: float, cached_len: int,
    ) -> torch.Tensor:
        """House adaptation #8a: the #5b v2 streaming loop re-blocked into
        fat fp16 tensor-core slices (the probed batched-streaming route,
        .boxpatch/boxprobe/kvarn-prefill-probe.py). The old body paid for
        the mask-safe shape with fp32 everywhere: q5/kbf/vbf/scf were fp32,
        the two matmuls ran TF32, and S/BLK iterations each launched ~14
        kernels -- at Lq=4096 the guard halved BLK to 1024, so a 100k
        chunk ran ~98 x 14 launches per layer. This version groups q per
        kv head ([Hk, R2=G*Lq, D] fp16, pre-scaled), takes one fp16
        tensor-core bmm per 4096-key slice, keeps the running max/sum and
        the accumulator in fp32 (the #5b accumulator discipline), does the
        exp in fp32 and casts p to fp16 only for the PV bmm. Probed on the
        box (deployment container, LQ=1024 ctx=100k history-only, D=512):
        117.7 ms/layer-chunk; numerics vs the fp32 gather+SDPA reference
        max abs 9.8e-4 / max rel 8.4e-4, 0 NaN. The fp16 score bmm is the
        one real precision delta vs the old body (scores rounded to fp16
        before the max/exp); the PPL probe gates it in production.

        Causal shape: query row i sits at global position cached_len + i
        and reads columns <= cached_len + i. Slices entirely below
        cached_len need no mask; every slice reaching past it gets the
        [Lq x slice] fill (nothing beyond exists -- this path's query
        range always ends at the sequence end). Row 0 of the first slice
        always has a visible column (column 0 <= cached_len + 0), so the
        running max is finite from slice 0 on, every row's l >= 1, and the
        rescale is NaN-free.

        Layout: returns [B1, Lq, Hq, D] (token-major) -- the only caller
        un-rotates with one contiguous reshape, no transpose copy.
        """
        B1, Hq, Lq, D = q.shape
        Hk = self.num_kv_heads
        G = Hq // Hk
        ...
        BHk = B1 * Hk
        R2 = G * Lq
        # Slice budget: the three score-shaped buffers are 2 + 4 + 2 =
        # 8 bytes per score element (scores fp16, p fp32, p16 fp16).
        # 512 MiB of them reproduces the probed configuration (Lq=1024 ->
        # 4096-key slices) and keeps the peak flat in context -- the #5b
        # v2 rule; the buffers are allocated once per call and reused in
        # place across slices. Under GPU-PV the allocator's segment reuse
        # does not save you: keep the working set flat and static.
        SLICE = 4096
        while SLICE > 512 and BHk * R2 * SLICE * 8 > 536870912:
            SLICE //= 2
        dev = q.device
        # grouped q, pre-scaled: r = g * Lq + token, so the causal mask
        # indexes the [.., G, Lq, ..] view of the score tensor on its Lq
        # dim. fp16 rounding of q*scale is the same class as the old
        # fp16-scale multiply of fp32 scores.
        q2 = q.reshape(B1, Hk, G, Lq, D).mul(scale).view(BHk, R2, D)
        scores = torch.empty(BHk, R2, SLICE, dtype=torch.float16, device=dev)
        p32 = torch.empty(BHk, R2, SLICE, dtype=torch.float32, device=dev)
        p16 = torch.empty(BHk, R2, SLICE, dtype=torch.float16, device=dev)
        pv = torch.empty(BHk, R2, D, dtype=torch.float16, device=dev)
        o = torch.zeros(BHk, R2, D, dtype=torch.float32, device=dev)
        m = torch.full((BHk, R2, 1), float("-inf"),
                       dtype=torch.float32, device=dev)
        m2 = torch.empty_like(m)          # running-max candidate
        a2 = torch.empty_like(m)          # alpha = exp(m - m2)
        l = torch.zeros(BHk, R2, 1, dtype=torch.float32, device=dev)
        qpos = (torch.arange(Lq, device=dev, dtype=torch.float32)
                .view(1, 1, 1, Lq, 1) + cached_len)
        for s0 in range(0, S, SLICE):
            s1 = min(s0 + SLICE, S)
            b = s1 - s0
            sb = scores[:, :, :b]
            # strided-gemm friendly: the k slice flattens to [BHk, b, D]
            # (a view: B1/Hk merge is stride-clean) with D innermost, so
            # the transposed operand needs no copy; sb (a prefix slice)
            # keeps row stride SLICE -- valid gemm ld.
            torch.bmm(q2, k[:, :, s0:s1].reshape(BHk, b, D).transpose(1, 2),
                      out=sb)
            if s1 > cached_len:
                kpos = torch.arange(s0, s1, device=dev).view(1, 1, 1, 1, b)
                sb.view(B1, Hk, G, Lq, b).masked_fill_(
                    kpos > qpos, float("-inf"))
            torch.maximum(m, sb.amax(dim=-1, keepdim=True), out=m2)
            torch.sub(sb, m2, out=p32[:, :, :b])
            p32[:, :, :b].exp_()
            p16[:, :, :b].copy_(p32[:, :, :b])
            a2.copy_(m).sub_(m2).exp_()           # alpha = exp(m - m2)
            l.mul_(a2).add_(p32[:, :, :b].sum(dim=-1, keepdim=True))
            torch.bmm(p16[:, :, :b], v[:, :, s0:s1].reshape(BHk, b, D),
                      out=pv)
            o.mul_(a2).add_(pv)
            m.copy_(m2)
        o = o / l
        # [BHk, R2, D] -> [B1, Hk, G, Lq, D] -> [B1, Lq, Hk, G, D]
        # -> [B1, Lq, Hq, D]: head h = kv*G + g preserved by the merge.
        return (o.view(B1, Hk, G, Lq, D).permute(0, 3, 1, 2, 4)
                .reshape(B1, Lq, Hq, D).to(q.dtype))
    def _prefill_first_chunk(
        ...
        # port(0.27.1): plain bool for the FA / SDPA kwarg (plan §2.1.8).
        causal = _causal_bool(getattr(attn_metadata, "causal", True))
        ...
        # Per-request SDPA fallback: head_dim > 256 (flash_attn caps at 256)
        # or no flash_attn. #5 keeps the fused mem-efficient kernel engaged
        # instead of falling to the math backend's full score matrix.
        outputs = []
    ...
    def _decode_path_slow(
        ...
        """Fallback: full fp16 dequant + SDPA in the rotated frame
        (#6 gather, #7 kernel gather). Used for multi-query-per-request
        decode steps (e.g. speculative decoding) and for head-512 contexts
        wider than the FA scratch, where the Triton kernel's ``(B, Hq)``
        per-program shape would mis-handle the per-token mapping.

        #7 semantic note: a context tail block WITHOUT a pool slot now
        reads its packed block like any other (the metadata builder
        guarantees a live tail has a pool slot -- the contract the fast
        path has always run on). The pre-#7 python gather zero-filled that
        ...
        # numbers are stage-true; the syncs themselves perturb pipelining
        # and must be off for any measurement that isn't this diagnosis).
        _tim = os.environ.get("KVARN_BT_TIMING") == "1"
        if os.environ.get("KVARN_BT_DEBUG") == "1":
            # Mark the route the 2x260k OOM came from. `_cached_multiquery_path`
            # reaches this only for head_dim > 256, for a single request wider
            # than the scratch, or for a grouped call that found no split — so a
            # line here whose summed context exceeds the scratch rows means the
            # fast path declined a batch it could have taken.
            _rows = int(self._fa_K_buf.shape[0]) if self._fa_K_buf is not None else -1
            print(f"[KVARN_SLOW] B={num_reqs} total_k={sum(seq_lens[:num_reqs])} "
                  f"rows={_rows} head_size={self.head_size} "
                  f"seq_lens={seq_lens[:num_reqs]}", flush=True)
        out = torch.empty(q.shape[0], self.num_heads, self.head_size,
                          dtype=q.dtype, device=q.device)
        # #7: the gather is ONE kernel launch per request now, not a python
        # loop. The old loop walked ~seq_len/group blocks (781 per layer at
        # 100k with group 128) each launching the per-tile dequant; this is
        # the same `_kvarn_build_packed_kv_kernel` the fast path packs its
        # FA scratch with every step -- pool slots, bit-width-aware packed
        # dequant and the masked partial tail handled inside, padding
        # programs exit at n_tok <= 0. MAX_BLOCKS_PER_REQ is a runtime int
        # (do_not_specialize), so the per-request grid value never
        # recompiles; the constexpr set matches `_warm_decode_kernels`
        # exactly, so the warm variant is the one that serves.
        cfg = self.kvarn_config
        group = cfg.group
        Hk = self.num_kv_heads
        from vllm.v1.attention.ops.triton_kvarn_decode import (
        ...
        # Fixed-address per-request scalars: rewriting a 1/2-element device
        # tensor per request keeps every launch's pointer arguments at the
        # same alignment, so Triton compiles ONE variant instead of one per
        # row-slice alignment (the fast path pays the same class of variants
        # on its row-sliced calls; here the fixed tensors are free).
        sl_one = torch.zeros(1, dtype=torch.int32, device=q.device)
        cu_k = torch.zeros(2, dtype=torch.int32, device=q.device)
        for r in range(num_reqs):
            ...
            if Lq == 1:
                # #8d: single-query decode steps run the packed-direct
                # kernel (the bit-clean-validated #8 prototype): no fp16
                # K/V ever materializes, no gather, one launch per layer.
                # The 09-21 timed boot measured the legacy route's stages
                # at 241.5 ms (gather) + 177.1 ms (attention) per layer
                # at a 100k context -- the wholesale ~840 MB pool rewrite
                # every layer every step is what the box's GPU-PV
                # residency punishes; the isolated probe's 16/11 ms
                # numbers do not survive it.
                from vllm.v1.attention.ops.triton_kvarn_prefill_direct import (
                    launch_prefill_direct,
                )
                sl_one[0] = seq_len
                q_rot = torch.mm(q[q_start:q_end].reshape(-1, D),
                                 H16).view(Lq, self.num_heads, D)
                o_rot = torch.empty(Lq, self.num_heads, D,
                                    dtype=torch.float16, device=q.device)
                launch_prefill_direct(
                    q_rot, attn_metadata.block_table[r:r + 1], sl_one,
                    self._block_to_slot_t, kv_cache,
                    self._tail_K_pool, self._tail_V_pool,
                    o_rot, self.scale, Lq,
                    num_heads=self.num_heads,
                ...
                o_true = torch.mm(o_rot.reshape(-1, D), H16).view(
                    Lq, self.num_heads, D)
                out[q_start:q_end] = o_true.to(q.dtype)
                if _tim:
                    torch.cuda.synchronize()
                    _t2 = time.perf_counter()
                    print(f"[KVARN_TIM] hs={self.head_size} Lq={Lq} "
                          f"seq={seq_len} "
                          f"direct={(_t2 - _t0) * 1e3:.1f}ms", flush=True)
                continue
            K_rot, V_rot = self._slow_kv_bufs(seq_len, q.device)
            max_blocks_r = (seq_len + group - 1) // group
            sl_one[0] = seq_len
            cu_k[1] = seq_len
            _kvarn_build_packed_kv_kernel[(max_blocks_r, Hk)](
                attn_metadata.block_table[r:r + 1],
                sl_one, cu_k, self._block_to_slot_t,
                ...
                self._tail_K_pool.stride(2),
                K_rot.stride(0), K_rot.stride(1),
                MAX_BLOCKS_PER_REQ=max_blocks_r,
                D=D, GROUP=group,
                K_BITS=cfg.key_bits, V_BITS=cfg.value_bits,
                NUM_BLOCKS_LOOKUP=self._block_lookup_size,
            ...
            # #5: fp16 end to end (the old .float() upcast doubled the
            # gather at long context). #5b: the masked call streams over
            # fixed K/V blocks instead of handing a bool mask to SDPA --
            # the mem-efficient kernel converts a mask into an internal
            # bias whose size scales with query x context (514-824 MiB at
            # the deep tail of a 189k prefill: the OOM that survived #5 at
            # any utilization). Streaming keeps the peak constant.
            # #6: work in the ROTATED frame the gather returns (the same
            # fp16 Hadamard as the store side and the fast decode path;
            # QK^T invariant). H16 is resolved above the #8d branch.
            q_r = q[q_start:q_end].transpose(0, 1).unsqueeze(0)
            q_rot = torch.mm(q_r.reshape(-1, D), H16).view(
                1, self.num_heads, Lq, D)
            K_t = K_rot.transpose(0, 1).unsqueeze(0)
            V_t = V_rot.transpose(0, 1).unsqueeze(0)
            cached_len = seq_len - Lq
            # #8a: token-major [1, Lq, Hq, D] -- the un-rotation below is a
            # plain reshape of the contiguous output, no transpose copy.
            o_rot = self._sdpa_streaming(
                q_rot, K_t, V_t, self.scale, cached_len,
            )                                                # [1, Lq, Hq, D]
            o_true = torch.mm(
                o_rot[0].reshape(-1, D), H16,
            ).view(Lq, self.num_heads, D)
            out[q_start:q_end] = o_true.to(q.dtype)
            if _tim:
                torch.cuda.synchronize()
                _t2 = time.perf_counter()
                print(f"[KVARN_TIM] hs={self.head_size} Lq={Lq} "
                      f"seq={seq_len} gather={(_t1 - _t0) * 1e3:.1f}ms "
                      f"attn={(_t2 - _t1) * 1e3:.1f}ms", flush=True)
        return out
    ...
    def _fused_verify_path(
        ...
        # port(0.27.1): pass the DEPLOYMENT-CONSTANT block bound (plan §1(b) /
        # I1 kernel change), never a per-step value: the driver forwards it as
        # the MAX_BLOCKS_PER_REQ constexpr, so a per-step
        # min(cdiv(max_seq_len, group), block_table.shape[1]) recompiled the
        # verify kernels every 128 tokens of context. Same value as
        # md.fa_max_blocks_per_req (= ceil(max_model_len / group)); computed
        # here because the prefill sub-metadata built by _mixed_batch_path does
        # not carry it. Correctness is unaffected — the kernels bound the block
        # loop by seq_len / vq_seqlen at runtime; the constant only sizes the
        # split-K decision.
        max_ctx_blocks = max((self._max_model_len + group - 1) // group, 1)
    ...
    def _cached_multiquery_path(
        ...
        path remains the fallback for head_dim > 256 (FA's cap) or for a batch
        carrying a single request wider than the scratch; an over-cap batch is
        split into scratch-fitting groups (house adaptation #4) so it stays on
        this route.
        """
        md = attn_metadata
        B = md.block_table.shape[0]
        ...
        # house adaptation #4: fit the batch to the scratch instead of falling
        # to the fp32 slow path. Measured on 2026-09-15: 2x260k summed to
        ...
        if total_k <= 0:
            return self._decode_path_slow(q, kv_cache, md)
        rows = int(self._fa_K_buf.shape[0])
        groups = _scratch_groups(seq_lens_list, rows)
        if groups is None:
            return self._decode_path_slow(q, kv_cache, md)
        if os.environ.get("KVARN_BT_DEBUG") == "1":
            # This path runs once per LAYER, so a long-ctx prefill prints
            # thousands of lines and a flat cap silently swallows the
            # interesting ones (measured: the cap fired 14 chunks into a 260k
            # prefill). Print every multi-group call, and sample the common
            # path so the log still shows a route heartbeat.
            _mn = KVarNAttentionImpl.__dict__.get("_mtq_dbg_n", 0)
            if len(groups) > 1 or _mn < 50:
                KVarNAttentionImpl._mtq_dbg_n = _mn + 1
                print(f"[KVARN_MTQ] {_mn} B={B} rows={rows} total_k={total_k} "
                      f"groups={groups} seq_lens={seq_lens_list}", flush=True)
        ...
        # port(0.27.1): fixed (deployment-constant) MAX_BLOCKS_PER_REQ, as in
        # the decode driver's materialize path — never the per-step
        # min(cdiv(max_seq_len, group), block_table.shape[1]) (that recompiled
        # this kernel on every distinct value, e.g. once per chunk of a long
        # chunked prefill). The extra (B * max_blocks) programs beyond a
        # request's context exit at their first instruction (n_tok <= 0
        # before any block-table load), so the wider grid is safe and cheap.
        max_blocks = (md.fa_max_blocks_per_req
                      or (self._max_model_len + group - 1) // group)
        ...
        qsl_t = md.query_start_loc[:B + 1]

        # house adaptation #4: one packed run + one varlen call per group. The
        # group's rows are re-based to local index 0 so the kernel's dst_base /
        # cu lookups land at the scratch's start; the row stride is the block
        # table's own, so slicing rows is a view, not a copy.
        def _pack_and_attend(r0, r1, q_g, cu_q_g, cu_k_g, g_k):
            _kvarn_build_packed_kv_kernel[((r1 - r0) * max_blocks, Hk)](
                md.block_table[r0:r1], seq_lens[r0:r1], cu_k_g,
                self._block_to_slot_t,
                kv_cache, self._tail_K_pool, self._tail_V_pool,
                K_packed, V_packed,
            ...
            return self._flash_varlen(
                q_g, K_packed[:g_k], V_packed[:g_k],
                cu_q=cu_q_g, cu_k=cu_k_g,
                max_q=md.max_query_len, max_k=max_k, causal=causal,
            )

        if len(groups) == 1:
            # The batch fits the scratch: the pre-change call, byte for byte —
            # one launch, one varlen call, the result used directly. No extra
            # allocation and no extra host sync beyond the seq_lens read above,
            # so nothing outside this path (the allocator's layout, the compiled
            # regions' inputs) can tell this build from the one before it.
            out_rot = _pack_and_attend(0, B, q_rot, qsl_t, cu_k, total_k)
        else:
            qsl = None
            ...
            for r0, r1 in groups:
                if qsl is None:
                    qsl = qsl_t.tolist()
                g_k = sum(seq_lens_list[r0:r1])
                cu_k_g = (cu_k[r0:r1 + 1] - cu_k[r0]).to(torch.int32)
                cu_q_g = (qsl_t[r0:r1 + 1] - qsl_t[r0]).to(torch.int32)
                out_rot[qsl[r0]:qsl[r1]] = _pack_and_attend(
                    r0, r1, q_rot[qsl[r0]:qsl[r1]], cu_q_g, cu_k_g, g_k,
                )
    ...
    def _mixed_batch_path(
        ...
        prefill_meta = KVarNMetadata(
            ...
            is_prefill=True,
            # port(0.27.1): carry the deployment-constant block bound so the
            # cached-multiquery route launches with a fixed MAX_BLOCKS_PER_REQ.
            fa_max_blocks_per_req=mbpr,
            fa_max_seqlen_k_fixed=self._max_model_len,
            causal=getattr(attn_metadata, "causal", True),
        )
