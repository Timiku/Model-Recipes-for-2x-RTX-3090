...
Triton kernels + a thin Python driver (materialize path calls FlashAttention).

  - ``_kvarn_scatter_store_kernel``: writes rotated fp16 k/v into the tail pool.
  - ``_kvarn_build_packed_kv_kernel``: block_table-driven dequant (int4 cache)
    + gather (fp16 tail pool) into a packed varlen ``[total_kv_tokens, Hk, D]``
    fp16 buffer that ``flash_attn_varlen_func`` consumes (materialize path).
  - ``_kvarn_fused_decode_kernel`` / ``_stage1`` / ``_stage2``: fused dual-source
    (int4 + pool) flash-decoding with online softmax (default path).
  - ``_kvarn_fused_verify_stage1``: shared-dequant spec-verify variant.
  - ``kvarn_decode_attention`` / ``kvarn_verify_attention``: drivers.

port(0.27.1): the superseded, never-called ``_kvarn_dequant_blocks_kernel``
(referenced the undefined ``K_BITS``) and ``_kvarn_pool_gather_packed_kernel``
were deleted.

Cache layout (per block, head) — ``KVarNConfig.tile_bytes_aligned`` bytes
(26880 for kvarn_k4v2_g128 @ head_dim 256), see KVarNConfig.

The task plan (block table, seq lens, block->pool-slot lookup) is built once
per batch in ``KVarNMetadataBuilder.build`` and reused across all attention
...
# port(0.27.1): MAX_BLOCKS_PER_REQ is a runtime int, but Triton still
# specialises integer args on value 1 and on divisibility by 16 — so the
# warmup's tiny n_blocks and serving's per-batch ceil(max_seq_len / group)
# would still land in different compiled variants (a JIT stall mid-serving
...
def _kvarn_build_packed_kv_kernel(
    ...
    # port(0.27.1): MAX_BLOCKS_PER_REQ is a plain RUNTIME int now (was
    # tl.constexpr). It is only used for the program-id -> (request, block)
    # div/mod below, and the chunked-prefill-continuation caller
    # (`_cached_multiquery_path`) passes a per-batch value (ceil(max_seq_len /
    # group)) that changes every chunk / every 128 tokens of context — as a
    # constexpr each new value was a fresh Triton compile. Kept the (uppercase)
    # name so all launch sites (`MAX_BLOCKS_PER_REQ=...` keyword) are unchanged.
    MAX_BLOCKS_PER_REQ,
    # constexprs
    D: tl.constexpr,
    GROUP: tl.constexpr,
...
def _kvarn_fused_decode_kernel(
    ...
    # constexprs
    # port(0.27.1): the unused `MAX_BLOCKS_PER_REQ: tl.constexpr` was dropped
    # from this kernel, `_kvarn_fused_decode_stage1` and
    # `_kvarn_fused_verify_stage1` (the block loop is bounded by seq_len, not by
    # it). As a constexpr it was part of the JIT cache key, and
    # `kvarn_verify_attention` receives a per-step `max_ctx_blocks` from
    # `_fused_verify_path` -> a fresh compile every 128 tokens of context.
    # Launch sites must NOT pass MAX_BLOCKS_PER_REQ to these three kernels.
    D: tl.constexpr,
    GROUP: tl.constexpr,
    ...
    for k in range(blk_lo, n_blocks):
        ...
        for c0 in range(0, GROUP, BLOCK_N):
            ...
            # port(0.27.1): NaN guard. While no key has been seen yet (fully
            # masked chunk(s) first, e.g. sliding-window start / an empty split
            # prefix) m_i == m_new == -inf and exp(-inf - -inf) = NaN would
...
def _kvarn_fused_decode_stage1(
    ...
    # port(0.27.1): unused MAX_BLOCKS_PER_REQ constexpr dropped (see the note
    # in _kvarn_fused_decode_kernel).
...
def _kvarn_fused_decode_stage2(
    ...
    # port(0.27.1): guard the combine against an ALL-EMPTY row (every split
    # saw zero KV positions -> every lse == -inf, e.g. a padded / zero-length
    # row under CUDA-graph capture or a fully-masked sliding window). Without
    # it `lse - g` is (-inf) - (-inf) = NaN and 0/0 poisons the output; with
    # it w == 0, denom == 0 -> out == 0. Non-empty rows: g is finite, g_safe ==
    # g and denom > 0, so the arithmetic is bit-identical to before.
    g_safe = tl.where(g == -float("inf"), 0.0, g)
    w = tl.exp(lse - g_safe)                                        # [SPLITS] weights
    ...
    out = tl.sum(w[:, None] * O, axis=0) / tl.where(denom > 0, denom, 1.0)  # [D]
...
def kvarn_decode_attention(
    ...
    For each layer call:
        ...
        2. Build a packed varlen ``[total_kv_tokens, Hk, D]`` fp16 K, V by
           dequantising the quantised blocks and copying sink + trailing-tail
           tokens from the rotated fp16 tail pool
           (``_kvarn_build_packed_kv_kernel``; materialize path only — the
           default fused path reads both sources in-kernel).
...
def kvarn_verify_attention(
    ...
    port(0.27.1): ``max_ctx_blocks`` is no longer forwarded to the kernels as
    a constexpr (they never used it); it only steers the Python-side split-K
    decision / split count below, so a per-step value from the caller
    (``_fused_verify_path``) is harmless (at most the two
    ``adaptive_num_kv_splits`` variants, 32 and 64, get compiled).
...
def _kvarn_fused_verify_stage1(
    ...
    for k in range(blk_lo, blk_hi):
        ...
        for c0 in range(0, GROUP, BLOCK_N):
            ...
            m_new = tl.maximum(m_i, tl.max(scores, axis=1))
            # port(0.27.1): NaN guard (see _kvarn_fused_decode_kernel). This is
            # the kernel where it actually bites: a Q row whose bottom-right
            # causal `limit` ends before this split's first block never sees a
            # live key, so m_i stays -inf for the whole split -> exp(-inf - -inf)
            # = NaN in l_i/acc -> NaN partial (the `nonempty` test below cannot
