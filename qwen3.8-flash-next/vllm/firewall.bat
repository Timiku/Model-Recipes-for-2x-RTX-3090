...
title model-recipes firewall - qwen3.8-flash-next
...
set "FW_OK=1"
set "FW_EXISTS="
for /f "delims=" %%O in ('netsh advfirewall firewall show rule name="model-recipes qwen3.8-flash-next 8115" 2^>nul ^| findstr /i "Rule Name:"') do set "FW_EXISTS=1"
if defined FW_EXISTS (
    echo    [ok] 8115 - the rule is already in place
) else (
    netsh advfirewall firewall add rule name="model-recipes qwen3.8-flash-next 8115" dir=in action=allow protocol=TCP localport=8115 profile=any >nul 2>nul
    if errorlevel 1 (
        set "FW_OK="
        echo    [--] 8115 - the add was refused
    ) else (
        echo    [ok] 8115 - added
    )
)
set "FW_EXISTS="
for /f "delims=" %%O in ('netsh advfirewall firewall show rule name="model-recipes qwen3.8-flash-next 8116" 2^>nul ^| findstr /i "Rule Name:"') do set "FW_EXISTS=1"
if defined FW_EXISTS (
    echo    [ok] 8116 - the rule is already in place
) else (
    netsh advfirewall firewall add rule name="model-recipes qwen3.8-flash-next 8116" dir=in action=allow protocol=TCP localport=8116 profile=any >nul 2>nul
    if errorlevel 1 (
        set "FW_OK="
        echo    [--] 8116 - the add was refused
    ) else (
        echo    [ok] 8116 - added
    )
)
...
if not defined FW_OK (
    ...
    echo    netsh advfirewall firewall add rule name="model-recipes qwen3.8-flash-next 8115" dir=in action=allow protocol=TCP localport=8115 profile=any
    echo    netsh advfirewall firewall add rule name="model-recipes qwen3.8-flash-next 8116" dir=in action=allow protocol=TCP localport=8116 profile=any
)
pause
exit /b 0
:not-elevated
