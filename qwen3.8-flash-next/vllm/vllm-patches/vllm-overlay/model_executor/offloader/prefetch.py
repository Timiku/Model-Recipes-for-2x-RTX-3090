    ...
    def start_onload_to_static(self):
        ...
        with torch.cuda.stream(self.copy_stream):
            for name, offloader in self._param_offloaders.items():
                ...
                # Empty Marlin sentinel tensors (for example, symmetric INT4
                # g_idx) have no storage to pin and PyTorch reports
                # is_pinned() == False even when allocated with pin_memory.
                # A zero-byte copy cannot introduce the synchronization this
                # guard is intended to prevent.
                assert (
                    cpu_storage.numel() == 0
                    or not should_pin_memory()
