...
def process_weights_after_loading(
    ...
    # Optional quant-method post-load initializers run only after every
    # device_loading_context above has restored UVA-backed parameters.  This
    # lets a cache mirror selected rows into VRAM without temporarily keeping
    # both the full layer and the mirror resident on the GPU.
    post_load_initializers = 0
    for module_name, module in model.named_modules():
        quant_method = getattr(module, "quant_method", None)
        initializer = getattr(quant_method, "maybe_init_static_hot_cache", None)
        if initializer is not None:
            # Some compressed-tensors MoE selectors do not forward their
            # already-available layer_name into the quant method.  Preserve
            # that identity here so a per-layer ranking cannot be applied to
            # the wrong module (in particular, the separate MTP draft).
            if hasattr(quant_method, "layer_name") and not quant_method.layer_name:
                quant_method.layer_name = module_name
            initializer(module)
            post_load_initializers += 1
    if post_load_initializers:
        logger.info(
            "Ran %d optional quant-method post-load initializer(s)",
            post_load_initializers,
        )
