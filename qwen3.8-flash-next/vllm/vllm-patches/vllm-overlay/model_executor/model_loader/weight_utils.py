...
def safetensors_weights_iterator(
    ...
    *,
    ...
    # A safetensors index is authoritative at the tensor level, not only at
    # the shard level.  In particular, a checkpoint view may supersede a
    # tensor with a newly quantized copy while reusing the original shard for
    # all of its other tensors.  filter_duplicate_safetensors_files() filters
    # whole files only, so without this check the stale physical tensor would
    # still be yielded and could overwrite or conflict with the indexed copy.
    indexed_names_by_file: dict[str, set[str]] | None = None
    parent_dirs = {os.path.dirname(os.path.abspath(path)) for path in sorted_files}
    if len(parent_dirs) == 1:
        checkpoint_dir = next(iter(parent_dirs))
        index_path = os.path.join(checkpoint_dir, SAFE_WEIGHTS_INDEX_NAME)
        if os.path.isfile(index_path):
            with open(index_path) as index_handle:
                weight_map = json.load(index_handle)["weight_map"]
            indexed_names_by_file = defaultdict(set)
            for name, relative_file in weight_map.items():
                indexed_file = os.path.abspath(
                    os.path.join(checkpoint_dir, relative_file)
                )
                indexed_names_by_file[indexed_file].add(name)
            logger.info_once(
                "Honoring tensor-level safetensors index entries from %s",
                index_path,
            )
    ...
    for st_file in tqdm(
        ...
        indexed_names = (
            indexed_names_by_file.get(os.path.abspath(st_file), set())
            if indexed_names_by_file is not None
            else None
        )
        if safetensors_load_strategy == "eager":
