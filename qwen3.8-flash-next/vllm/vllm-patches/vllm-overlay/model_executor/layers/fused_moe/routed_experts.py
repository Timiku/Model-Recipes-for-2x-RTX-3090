    ...
    def weight_loader(
        ...
        if "scale" in weight_name or "zero" in weight_name or "offset" in weight_name:
            ...
            if quant_method == FusedMoeWeightScaleSupported.CHANNEL.value:
                ...
                group_scale_refine = getattr(param, "weight_scale_refine", 1)
                if group_scale_refine > 1:
                    # A group-128 symmetric checkpoint can be represented
                    # exactly as group-64 by duplicating each scale.  This is
                    # useful when a TP shard boundary cuts a checkpoint group:
                    # packed INT4 weights remain byte-identical while each
                    # rank receives a kernel-compatible local scale grid.
                    group_dim = 0 if is_transposed else -1
                    loaded_weight = loaded_weight.repeat_interleave(
                        group_scale_refine, dim=group_dim
                    )
