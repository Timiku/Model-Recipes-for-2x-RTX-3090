...
def get_routed_experts_attn_gid(kv_cache_config: KVCacheConfig) -> int:
    ...
    for gid, group in enumerate(kv_cache_config.kv_cache_groups):
        ...
        if isinstance(group_spec, UniformTypeKVCacheSpecs) and any(
            ...
            return gid
    # Out-of-tree attention backends can register a custom KVCacheSpec even
    # when their one and only cache group is the token-indexed autoregressive
    # cache. Qwen3.8 Flash-Next's QSA state backend is such a case. Its slot
    # mapping is exactly the mapping the routing recorder needs, so accept the
    # unambiguous single-group configuration.
    if len(kv_cache_config.kv_cache_groups) == 1:
        logger.warning(
            "Routed-experts capture: using sole custom KV cache group %s",
            type(kv_cache_config.kv_cache_groups[0].kv_cache_spec).__name__,
        )
        return 0
