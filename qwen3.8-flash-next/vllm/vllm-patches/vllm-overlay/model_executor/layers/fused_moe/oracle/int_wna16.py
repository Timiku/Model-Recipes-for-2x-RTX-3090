...
def _humming_wna16_weight_schema(
    ...
    if isinstance(quant_config, QuantizationArgs):
        ...
        if hasattr(actorder, "value"):
            actorder = actorder.value
        return {
...
class _CompressedTensorsKFirstHummingWeightSchema:
    """Adapter from vLLM's K-first CT MoE packing to Humming's N-first layout.

    The generic compressed-tensors MoE loader stores packed weights as
    ``[E, K / pack_factor, N]`` and group scales as ``[E, K / group_size, N]``.
    Humming's checkpoint schema expects both tensors with N before K.  Going
    through ``CompressedTensorsWeightSchema`` directly therefore validates the
    correct data against the wrong shapes.
    """

    def __init__(self, bits: int, group_size: int, symmetric: bool) -> None:
        if not symmetric:
            raise NotImplementedError(
                "Humming's K-first compressed-tensors MoE adapter currently "
    ...
    def convert_humming(
        ...
        output = {
            "weight": tensors["weight_packed"].transpose(-1, -2).contiguous(),
...
def convert_to_wna16_moe_kernel_format(
    ...
    if backend == WNA16MoEBackend.HUMMING:
        ...
        if isinstance(quant_config, MoeWNA16Config):
            ...
            convert_to_humming_moe_kernel_format(
                layer,
                weight_schema=_CompressedTensorsKFirstHummingWeightSchema(
                    bits=quant_config.num_bits,
                    group_size=quant_config.group_size,
                    symmetric=quant_config.symmetric,
                ),
                input_schema=HummingInputSchema(),
