...
class QwenGatedDeltaNetAttention(GatedDeltaNetAttention):
    ...
    def __init__(
        ...
        offload_process = is_offload_process()
        self.norm = RMSNormGated(
            ...
            # The PLE CPU worker constructs a meta-only skeleton of the
            # surrounding transformer. Avoid materializing this unrelated GDN
            # norm (and a CUDA context) in that process.
            device=(
                torch.device("meta")
                if offload_process
        ...
        if offload_process:
            # No transformer forward ever runs in the dedicated PLE worker.
            # Backend selection probes CUDA capability, so skip it while the
            # meta model is used solely to discover PLE submodules.
