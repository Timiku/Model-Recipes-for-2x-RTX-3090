...
class Fp8PerTensorOnlineMoEMethod(_Fp8OnlineMoEBase):
    ...
    def process_weights_after_loading(self, layer: Module) -> None:
        ...
        # The Marlin conversion below allocates another packed copy of each
        # FP8 tensor. Drop the now-unused BF16 expert storage first so online
        # quantization does not momentarily require BF16 + FP8 + packed FP8.
        old_w13 = layer.w13_weight
        old_w2 = layer.w2_weight
        old_w13.data = torch.empty(0, dtype=old_w13.dtype, device=old_w13.device)
        old_w2.data = torch.empty(0, dtype=old_w2.dtype, device=old_w2.device)
        replace_parameter(layer, "w13_weight", w13)
        replace_parameter(layer, "w2_weight", w2)
        del old_w13, old_w2
