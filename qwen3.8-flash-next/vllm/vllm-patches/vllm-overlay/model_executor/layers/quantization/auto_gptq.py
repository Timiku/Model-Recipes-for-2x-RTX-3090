...
def get_moe_quant_method(
    ...
    # Keep the model prefix on the quant method as well as the RoutedExperts
    # layer.  The offloaded-expert ranking file is keyed by transformer layer.
...
class AutoGPTQMoEMethod(FusedMoEMethodBase):
    ...
    def __init__(
        ...
        # The expert hot-cache implementation was originally added to the
        # compressed-tensors WNA16 loader.  Expose its format-independent
        # fields here so Intel's native AutoRound/GPTQ packing can use the same
        # post-conversion cache without repacking the checkpoint.
        self.wna16_backend = self.wna16_moe_backend
        self.symmetric = self.quant_config.is_sym
        self.num_bits = self.quant_config.weight_bits
        self.group_size = self.quant_config.group_size
        self.is_marlin = self.wna16_moe_backend in (
            WNA16MoEBackend.MARLIN,
            WNA16MoEBackend.BATCHED_MARLIN,
        )
        self.layer_name = ""
        self._static_hot_cache = None
    ...
    def process_weights_after_loading(self, layer: RoutedExperts) -> None:
        ...
        if converted is None:
            ...
            if self.wna16_moe_backend == WNA16MoEBackend.HUMMING:
                ...
                # parameters under their original checkpoint names.  Keep the
                # transformed Humming weights under AutoRound's *_qweight
                # names until the model-level cache initializer runs.
    ...
    def _cache_expert_rows(
        ...
        return CompressedTensorsWNA16MoEMethod._cache_expert_rows(
            tensor, local_ids, local_num_experts
        )
    ...
    def _gather_lru_rows(
        ...
        CompressedTensorsWNA16MoEMethod._gather_lru_rows(
            source, output, miss_local_ids, miss_slots, num_ids
        )

    def maybe_init_static_hot_cache(self, layer: RoutedExperts) -> None:
        """Initialize the shared Humming hot cache after UVA offload restoration."""
        if self.wna16_moe_backend != WNA16MoEBackend.HUMMING:
            return

        # Translate AutoRound's checkpoint names to the generic names used by
        # the format-independent cache.  Humming already standardized scales.
        ...
        CompressedTensorsWNA16MoEMethod.maybe_init_static_hot_cache(self, layer)
