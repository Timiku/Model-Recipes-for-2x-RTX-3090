...
# model-recipes local overlay delta (upstream vllm-project/vllm#55054, merged
# 2026-09-03): the five spec/non-spec/decode request-index transfers in
# PleShortConvAttentionMetadataBuilder.build use async_tensor_h2d instead of
# synchronous .to(device), so the per-step PLE metadata build no longer forces
# a cudaStreamSynchronize (it is the fixed term of this box's WSL2 decode
# step). The 2026-08-26 vendor pin predates the merge, so the file is the
# pin's copy with the PR's hunks applied verbatim. See upstream/SYNC.md.
