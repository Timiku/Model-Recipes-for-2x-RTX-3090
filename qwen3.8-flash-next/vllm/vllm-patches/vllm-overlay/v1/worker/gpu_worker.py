...
class Worker(WorkerBase):
    ...
    def _has_ple_layers(self) -> bool:
        ...
        # Only pipeline stages that own a PLE layer should register buffers
        # with the node-local offload worker.  Rank ordering is TP-fastest,
        # matching the PP-rank calculation used elsewhere in vLLM.
        parallel_config = self.parallel_config
        pp_rank = (
            self.rank // parallel_config.tensor_parallel_size
        ...
        return any(start_layer <= layer_id < end_layer for layer_id in ple_layer_ids)
