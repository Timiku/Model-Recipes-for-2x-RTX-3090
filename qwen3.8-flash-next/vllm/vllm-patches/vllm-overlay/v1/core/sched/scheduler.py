...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        self.force_dynamic_spec_scheduling = os.getenv(
            "VLLM_FORCE_DYNAMIC_SPEC_SCHEDULING", "0"
        ).lower() in ("1", "true", "yes")
        ...
        if speculative_config is not None:
            ...
            elif self.force_dynamic_spec_scheduling and self.num_spec_tokens > 0:
                # A non-None lookup selects vLLM's variable-K scheduling path.
                # Keep the configured depth for every supported batch size.
                # Qwen3.8 benefits from the variable-K scheduler mechanics on
                # this host even when K itself remains fixed.
                self.dynamic_sd_lookup = [0] + [
                    self.num_spec_tokens
                ] * self.scheduler_config.max_num_seqs
                logger.info(
                    "Using variable-K scheduler mechanics with fixed K=%d",
                    self.num_spec_tokens,
                )
