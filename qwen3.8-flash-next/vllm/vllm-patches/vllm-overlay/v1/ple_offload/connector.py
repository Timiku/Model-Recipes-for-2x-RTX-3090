...
class PleOffloadConnector:
    ...
    def __init__(
        ...
        *,
        ...
        self._layers = self._setup_layers(model)
        ...
        # The CPU worker writes PLE results into shared host buffers. This GPU
        # worker then performs H2D using its existing CUDA context, avoiding a
        # separate ~300 MiB CUDA context per GPU in the CPU worker.
        max_num_tokens = scheduler_config.max_num_batched_tokens
        embedding_dim = int(config.ple_embed_dim)
        self._cpu_output_buffers = {
            name: torch.empty(
                max_num_tokens,
                embedding_dim,
        ...
        self._cpu_output_ready_flags = {
            name: torch.zeros(1, dtype=torch.int32, device="cpu").share_memory_()
            for name in self._layers
        }
        self._cpu_output_consumed_flags = {
            name: torch.ones(1, dtype=torch.int32, device="cpu").share_memory_()
            for name in self._layers
        }
        self._ready_device_ptrs: dict[str, int] = {}
        self._consumed_device_ptrs: dict[str, int] = {}
        ...
        try:
            ...
            with torch.accelerator.device_index(self.device.index):
                self._pin_output_buffers()
                self._attach_output_buffers(vllm_config)
                if self.tp_rank == 0:
    ...
    def _setup_layers(
        ...
        """Discover GPU-side PLE placeholders."""
        layers = {
            name: module
    ...
    def _attach_output_buffers(self, vllm_config: VllmConfig) -> None:
        """Attach mapped-host inputs and device outputs to PLE placeholders."""
        config = vllm_config.model_config.hf_text_config
        ...
        for name, layer in self._layers.items():
            ...
            layer.setup_cross_process_offload(
                output_buffer,
                self._cpu_output_buffers[name],
                self._ready_device_ptrs[name],
                self._consumed_device_ptrs[name],
            )
    ...
    def _pin_output_buffers(self) -> None:
        """Page-lock outputs and map their handshake flags into CUDA."""
        for name, buffer in self._cpu_output_buffers.items():
            if buffer.device.type != "cpu" or not buffer.is_shared():
                raise RuntimeError("PLE output buffers must be shared CPU tensors")
            if not buffer.is_contiguous():
                raise RuntimeError("PLE output buffers must be contiguous")
            _cuda_check(
                cuda_driver.cuMemHostRegister(
                    buffer.data_ptr(),
                    buffer.numel() * buffer.element_size(),
                ...
                "cuMemHostRegister(PLE output buffer)",
            )
            self._pinned_host_buffers.append(buffer)
            if not buffer.is_pinned():
                raise RuntimeError("CUDA did not page-lock a PLE output buffer")

            for kind, flag, pointers in (
                ("ready", self._cpu_output_ready_flags[name], self._ready_device_ptrs),
                (
                    "consumed",
                    self._cpu_output_consumed_flags[name],
                    self._consumed_device_ptrs,
                ),
            ):
                _cuda_check(
                    cuda_driver.cuMemHostRegister(
                        flag.data_ptr(),
                        flag.numel() * flag.element_size(),
                        cuda_driver.CU_MEMHOSTREGISTER_PORTABLE
                        | cuda_driver.CU_MEMHOSTREGISTER_DEVICEMAP,
                    ),
                    f"cuMemHostRegister(PLE {kind} flag)",
                )
                self._pinned_host_buffers.append(flag)
                result = _cuda_check(
                    cuda_driver.cuMemHostGetDevicePointer(flag.data_ptr(), 0),
                    f"cuMemHostGetDevicePointer(PLE {kind} flag)",
                )
                pointers[name] = int(result[1])
    ...
    def _register_with_offload_worker(
        ...
        """Register shared CPU outputs and inputs with the worker."""
        ...
        registration = PleOffloadRegistration(
            ...
            cpu_output_buffers=self._cpu_output_buffers,
            cpu_output_ready_flags=self._cpu_output_ready_flags,
            cpu_output_consumed_flags=self._cpu_output_consumed_flags,
            input_ids_buf=self._input_ids_buf,
            query_start_loc_buf=self._query_start_loc_buf,
            ngram_context_buf=self._ngram_context_buf,
        )

        # ForkingPickler transmits only CPU shared-memory tensors. The CPU
        # offload process must never receive a CUDA tensor.
    ...
    def _process_request(self, request: PleOffloadRequest, socket: zmq.Socket) -> None:
        """Stage inputs and publish one CPU request without touching CUDA H2D."""
        if self._uses_cuda_inputs:
    ...
    def _launch(
        ...
        # TP0 publishes the CPU request. Every TP rank waits only until the
        # CPU worker has claimed its host output, preventing a stale ready=1
        # from satisfying the captured model-stream wait.
        if self.tp_rank == 0 and self._uses_cuda_inputs:
        ...
        for name, consumed_flag in self._cpu_output_consumed_flags.items():
            while int(consumed_flag.item()) != 0:
                if time.monotonic() >= deadline:
                    raise TimeoutError(f"Timed out claiming PLE output {name}")
                time.sleep(0)
    ...
    def signal_dummy_outputs(self, num_tokens: int) -> None:
        ...
        for name, buffer in self._cpu_output_buffers.items():
            ...
            self._cpu_output_consumed_flags[name].fill_(1)

    def release_outputs(self) -> None:
        """Compatibility hook; the model-stream copy releases host outputs."""
