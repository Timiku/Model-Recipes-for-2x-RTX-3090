...
if not hasattr(envs, "VLLM_PLE_DISK_OFFLOAD_DIR"):
    # The pinned image's vllm.envs predates VLLM_PLE_DISK_OFFLOAD_DIR
    # (upstream #54070): backfill it from the environment so the disk
    # tier works on this pin. The backfilled attribute is read-only here,
    # and the offload worker is the only process that consumes it - the
    # backfill lives in this module, never in the image's envs file.
    import os

    envs.VLLM_PLE_DISK_OFFLOAD_DIR = os.getenv(  # type: ignore[attr-defined]
...
def _disk_backed_tensor(path: str, shape: tuple[int, ...], dtype: torch.dtype,
    ...
    MADV_RANDOM is applied either way: gathers hit the kernel page cache, so
    steady-state residency follows the actual PLE working set rather than table
    size, and readahead only evicts useful pages.
    """
    import numpy as np
