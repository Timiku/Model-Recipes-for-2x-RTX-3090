...
def _hc_project_gate_mix(
    ...
    """Project HC gates and consume them without a full-token gate buffer.

    The unchunked up projection materializes ``[M, HC * H]``.  At an 8K
    prefill that is a 160 MiB BF16 temporary for Qwen3.8-Flash-Next.  Gate
    mixing consumes rows independently, so large prefills can be split along
    the token dimension while preserving the original GEMM and Triton kernel
    for each slice.  Decode stays on the original single-call fast path.
    """
    num_tokens, hyper_hidden_size = x.shape
    assert weight.shape[0] == hyper_hidden_size
    assert weight.shape[1] == lora.shape[1]
    assert num_tokens == lora.shape[0]
    ...
    out = x.new_empty((num_tokens, hidden_size))
    for start in range(0, num_tokens, chunk_tokens):
        end = min(start + chunk_tokens, num_tokens)
        gate = F.linear(lora[start:end], weight)
        mixed = _hc_gate_mix(x[start:end], gate, hc_count)
        out[start:end].copy_(mixed)
    return out
...
direct_register_custom_op(
    op_name="qwen3_8_flash_next_grouped_gemma_rmsnorm",
    op_func=_grouped_gemma_rmsnorm,
    fake_impl=_same_shape_fake,
)
direct_register_custom_op(
    op_name="qwen3_8_flash_next_hc_silu",
    op_func=_hc_silu,
    fake_impl=_same_shape_fake,
)
direct_register_custom_op(
    op_name="qwen3_8_flash_next_hc_gate_mix",
    op_func=_hc_gate_mix,
    fake_impl=_hc_gate_mix_fake,
)
direct_register_custom_op(
    op_name="qwen3_8_flash_next_hc_project_gate_mix",
    op_func=_hc_project_gate_mix,
    fake_impl=_hc_project_gate_mix_fake,
)
direct_register_custom_op(
    op_name="qwen3_8_flash_next_hc_combine",
    op_func=_hc_combine,
    fake_impl=_hc_combine_fake,
)
direct_register_custom_op(
    op_name="qwen3_8_flash_next_hc_combine_norm",
...
def grouped_gemma_rmsnorm(
    ...
    return torch.ops.vllm.qwen3_8_flash_next_grouped_gemma_rmsnorm(
        x, weight, eps, num_groups
    )


def hc_silu(x: torch.Tensor, hc_count: int) -> torch.Tensor:
    return torch.ops.vllm.qwen3_8_flash_next_hc_silu(x, hc_count)


def hc_gate_mix(x: torch.Tensor, gate: torch.Tensor, hc_count: int) -> torch.Tensor:
    return torch.ops.vllm.qwen3_8_flash_next_hc_gate_mix(x, gate, hc_count)


def hc_project_gate_mix(
    x: torch.Tensor,
    lora: torch.Tensor,
    weight: torch.Tensor,
    hc_count: int,
    ...
    return torch.ops.vllm.qwen3_8_flash_next_hc_project_gate_mix(
        x, lora, weight, hc_count, chunk_tokens
    )
...
def hc_combine(
    ...
    return torch.ops.vllm.qwen3_8_flash_next_hc_combine(
...
def hc_combine_norm(
    ...
    return torch.ops.vllm.qwen3_8_flash_next_hc_combine_norm(
        residual,
        block_output,
        injection_logits,
        norm_weight,
