...
if _QSA_TOPK_MODE in ("true", "yes"):
    _QSA_TOPK_MODE = "1"
_QSA_COLS_CACHE: dict[tuple[int, torch.device], torch.Tensor] = {}


def _qsa_cols(columns: int, device: torch.device) -> torch.Tensor:
    key = (columns, device)
    result = _QSA_COLS_CACHE.get(key)
    if result is None:
        result = torch.arange(columns, device=device, dtype=torch.int32)
        _QSA_COLS_CACHE[key] = result
    return result


def _qsa_mask_invisible_(
    logits: torch.Tensor, visible_blocks: torch.Tensor, columns: int
) -> None:
    """Mask columns the scoring kernel intentionally leaves unwritten."""

    cols = _qsa_cols(columns, logits.device)
    logits[:, :columns].masked_fill_(
        cols[None, :] >= visible_blocks[:, None], float("-inf")
    )
...
def _qsa_exact_topk(
    ...
    """Select exact visible QSA blocks with deterministic score ordering."""

    _qsa_mask_invisible_(logits, visible_blocks, columns)
    k = min(block_topk, columns)
    # Sorting is required when visible_blocks[row] < k. In that case topk also
    # returns masked -inf entries; finite/visible indices must precede padding
    # because the expansion kernel only consumes the first visible entries.
    indices = torch.topk(logits[:, :columns], k, dim=1, sorted=True).indices
    if k < block_topk:
        blocks[:, :k].copy_(indices)
        blocks[:, k:].fill_(-1)
    else:
        blocks.copy_(indices)
