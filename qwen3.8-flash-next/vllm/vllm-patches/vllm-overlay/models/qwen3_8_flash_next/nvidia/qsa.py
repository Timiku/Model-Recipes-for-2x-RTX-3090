    ...
    def __init__(
        ...
        *,
        ...
        if topk_indices_buffer is None:
            ...
            or topk_indices_buffer.shape[1] < self.indexer.output_width
        ):
            raise ValueError(
                "QSA shared top-k buffer must be INT32 with shape at least "
                f"({max_tokens}, {self.indexer.output_width}), got "
                f"{tuple(topk_indices_buffer.shape)} {topk_indices_buffer.dtype}"
            )
        # QSA layers execute serially, so one stable scratch arena can be
        # reused by every layer (and by MTP) without retaining one ~64 MiB
        # allocation per layer at an 8K scheduler batch.
