...
def _make_draft_vllm_config(
    ...
    # The target cache planner may already have derived a hybrid-model Mamba
    # page size by the time the MTP model is materialized. Revalidating that
    # value against a capped draft max_model_len makes it look like a
    # user-specified --mamba-block-size. Clear it only during validation. The
    # cache object must remain shared: platform cache alignment later updates
    # its attention block size, and QSA's main/compressed/state owners must all
    # observe the same resolved geometry.
    cache_config = vllm_config.cache_config
    target_mamba_block_size = cache_config.mamba_block_size
    ...
    try:
        ...
        cache_config.mamba_block_size = target_mamba_block_size
    draft_vllm_config.cache_config = cache_config
