...
class Qwen3_8FlashNextDecoderLayer(nn.Module):
    def __init__(
        ...
        # The dedicated PLE worker constructs this model only to discover and
        # load PleOffloadLayer subtrees. Omitting unrelated attention, MoE, and
        # hyperconnection modules both speeds discovery and guarantees that a
        # nominally CPU-only process cannot create a CUDA context through
        # backend selection or capability probes.
    ...
    def set_moe_parameters(self, layers: Iterable[nn.Module]) -> None:
        ...
        for layer in layers:
            ...
            if isinstance(layer, Qwen3_8FlashNextDecoderLayer) and isinstance(
                mlp, Qwen3_8FlashNextSparseMoeBlock
            ):
                example_moe = mlp
...
class Qwen3_8FlashNextModel(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        if self._qsa_layer_ids and not is_offload_process():
            max_tokens = vllm_config.scheduler_config.max_num_batched_tokens
            output_width = (
                config.indexer_budget + config.indexer_compress_ratio - 1
            )
            # All QSA layers run sequentially and overwrite this scratch arena
            # before consuming it. Sharing mirrors DeepSeek V4's layout and
            # also lets the existing MTP wiring reuse the target buffer.
            self.topk_indices_buffer: torch.Tensor | None = torch.empty(
                max_tokens,
                output_width,
                dtype=torch.int32,
            )
        ...
        self.hyper_connection_mixer: GatedResidual | PPMissingLayer
...
class Qwen3_8FlashNextForCausalLM(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        # The dedicated PLE worker only uses this model as a meta skeleton for
        # locating PLE submodules. The capability probe would otherwise create
        # an unnecessary CUDA context in that CPU-only process.
