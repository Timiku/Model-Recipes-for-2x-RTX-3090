...
class QSAMetadataBuilder(AttentionMetadataBuilder[QSAForwardMetadata]):
    ...
    # The Triton metadata builder can be replayed between fused draft steps.
    # All step-dependent inputs point at persistent draft input buffers, while
    # the derived QSA metadata is written into builder-owned persistent buffers.
    ...
    def update_draft_decode_metadata(self, metadata: QSAForwardMetadata) -> None:
        """Refresh position- and slot-dependent QSA metadata in place.

        The fused draft loop advances ``seq_lens`` and refreshes the common
        slot mapping before calling this method.  Relaunching the same Triton
        metadata kernel updates the persistent QSA buffers and is safe to
        capture as part of the fused multi-step CUDA graph.
        """
        assert HAS_TRITON
        ...
        build_k_work = not self.is_circular_buffer and self.compress_ratio != 1
        k_work_metadata = metadata.k_work_metadata if build_k_work else None
        max_num_work = metadata.k_work_metadata.shape[0] if build_k_work else 0
        request_scan_size = (
            1 << int(math.ceil(math.log2(self.request_capacity)))
            if build_k_work
            else 1
        )
        num_token_blocks = cdiv(num_tokens, 128)
        num_work_blocks = cdiv(max_num_work, 256) if build_k_work else 0
        ...
        _build_qsa_metadata_kernel[(max(num_token_blocks, num_work_blocks, 1),)](
            ...
            metadata.block_table.stride(1),
            num_reqs,
            num_mapped_tokens,
            num_tokens,
            max_num_work,
            int(math.ceil(math.log2(num_reqs))),
            int(math.ceil(math.log2(num_reqs))),
